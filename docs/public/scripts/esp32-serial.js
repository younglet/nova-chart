 ...
 *
 * 用途：在浏览器里经 Web Serial API 把 nova-chart.min.js 等前端文件
 * 写到 ESP32 的 /static/ 目录下。
 *
 * 项目特定说明（nova-chart）：
 *   - 主要写 nova-chart.min.js + nova-chart.min.css + index.novachart.html
 *   - HTML 文件名加 .novachart. 前缀，避免多项目写到同一 ESP32 时
 ...
 *     await esp.connect();
 *     await esp.writeFile('/static/nova-chart.min.js', bytes, pct => {});
 *     await esp.disconnect();
 *   </script>
 *
 * 修改记录：
 *   2026-06-21  从 ssd1306_view 复制到 nova-frontend/nova-chart/scripts/，
 *               上方 banner 改为 nova-chart 特定说明，源码不动。
 */
